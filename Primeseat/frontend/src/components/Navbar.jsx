 ...
 * 
 * Renders the top navigation header pinned sticky at the top of every screen.
 * Leverages the AuthContext to render links conditionally (Login/Signup vs Dashboard/Logout).
 * Features standard Tailwind utility classes, styling overlays, and an animated hover logout trigger.
 */
const Navbar = () => {
    const { user, logout } = useContext(AuthContext);
    const navigate = useNavigate();

    // Trigger user logout action and redirect back to sign-in page
    const handleLogout = () => {
        logout();
        navigate('/login');
    };
            ...
            <div className="container mx-auto px-4">
                <div className="flex flex-col md:flex-row justify-between items-center py-4 gap-4">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
                        ...
                        {user ? (
                            <>
                                {/* Redirects to AdminDashboard if user is admin, else standard UserDashboard */}
                                ...
                                {/* Micro-animated Expandable Logout Button */}
                                <button
                                ...
                                >
                                    ...
                                    <div className="absolute right-3.5 transform translate-x-full opacity-0 text-white text-sm font-medium transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100">
                                        Logout
                                    </div>
                                </button>
                            </>
